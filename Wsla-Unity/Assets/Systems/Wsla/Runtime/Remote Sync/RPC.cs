...
namespace Wsla.Unity
{
    ...
    public abstract class BaseRpcBind
    {
        ...
        internal abstract void Invoke<TStream>(ref TStream reader, RpcInfo info)
    ...
    public struct RpcInfo
    {
        ...
        public RpcInfo(NetworkClient Sender, byte Channel, DeliveryMethod Delivery)
        {
            this.Sender = Sender;
        ...
        public static RpcInfo From(RoomInstance room, ref NetworkRpcCommand command, byte channel, DeliveryMethod delivery)
        {
            ...
            return new RpcInfo(sender, channel, delivery);
        }
        ...
        public static RpcInfo From(ref RpcInvocationBuilder builder)
        {
            ...
            return new RpcInfo(sender, builder.Channel, builder.Delivery);
        }
    }
    ...
    public struct RpcInvocationBuilder
    {
        ...
        NetworkRpcParameters WriteArguments(NetDataWriter writer)
        {
            var payload = new NetworkRpcParameters(Bind.Entity.ID, Bind.Behaviour.ID, Bind.ID);

            if (ArgumentsWriter.Length > 0)
            {
                var destination = writer.Take(ArgumentsWriter.Length);
                ArgumentsWriter.CopyTo(destination);
            }
        ...
        void InvokeLocal()
        {
            ...
            Bind.Invoke(ref ArgumentsWriter, info);
        }
